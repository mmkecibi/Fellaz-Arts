    ...
    <form>
      ...
      <div class="col-md-6">
        ...
        <label>{{ $t("photo") }}</label>
          ...
          <button type="button" class="btn btn-primary" @click="editSlider">{{ $t("update") }}</button>
      </form>
...
export default {
    ...
    async fetch({ store, params }) {
          await store.dispatch("product/fetchSliderById", params.id);
        },
        data(){
      return {
              categories:[],
    ...
    created(){
                this.form.id = this.formslider.id;
                this.form.title = this.formslider.title;
                this.form.message = this.formslider.message; 
                this.form.captures= [... this.formslider.photo];
    },
    ...
    formslider: {
        get: function () {
          let arryobjslider = {...this.$store.state.product.slider};
          return arryobjslider;
        },
      },
    },
  ...
  async  editSlider () {
    ...
    this.form.captures = JSON.stringify(this.imageslocal);
      this.$store
        .dispatch("product/updateSlider",this.form)
        .then((result) => {   
    ...
    uploadImageSuccess(formData, index, fileList) {
      if (!this.imageExists(index, fileList)) {
        this.readURL(fileList[index].path,300).then((finalImg) => { this.imageslocal.push(finalImg); })
      }
