<template>
  <div class="addProduct container">
    <span class="title">{{ $t("Update Product") }}</span>

      <div class="credit-card">

      <div class="form-header">
        <h4 class="title">{{ $t("Image detail") }}</h4>
      </div>
...
colors:[
 ...
 validations: {
  ...
  computed: {
    ...
    created(){
                console.log(' this.formproduct.productcolorphotos xxx : ')

                this.formproduct.productcolorphotos.forEach(el => {
                   let attrobj = {};
                    attrobj.id = el.id;
                    attrobj.name = el.name;
                    attrobj.color = el.color;
                    attrobj.extraprice = el.extraprice;
                    attrobj.productid = el.productid;
                    attrobj.isdisplay = el.isdisplay;
                    attrobj.captures = JSON.parse(el.photo);
